...
public class Lexer{
	...
	**/
	public Lexer (String fileName) throws FileNotFoundException {
		...
		keywordsTable.add(new Token(ADDITION, "+", 0));
		keywordsTable.add(new Token(MULTIPLICATION, "*", 0));
		keywordsTable.add(new Token(CONSTANT, "final", 0));
		keywordsTable.add(new Token(WHILE, "while", 0));
		keywordsTable.add(new Token(ENDWHILE, "endwhile", 0));
		keywordsTable.add(new Token(NUMBER, "INTEGER", 0));
    keywordsTable.add(new Token(VARIABLE, "VARIABLE", 0));
		keywordsTable.add(new Token(CONSTANT, "CONSTANT", 0));
	...
	**/
	...
	public String getText(int code){
		if (code == INVALIDTOKEN) return "INVALIDTOKEN";
		for (Token token : keywordsTable) {
			if (token.code == code)
				return token.text;
		}
		return " ";
	...
	**/
	public int getTokenCode(String tokenText)
	{
		...
		while ((it.hasNext()) && (tokenCode==-1)){
			...
			if(tokenText.charAt(0) >= 48 && tokenText.charAt(0) <= 57){
				try {
		...
		if (tokenCode ==-1)
			if (tokenText.equals(tokenText.toUpperCase())) {
				tokenCode = CONSTANT;
			}else{
				tokenCode = VARIABLE;
			}
